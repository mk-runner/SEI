...
class GlobalPretrain(nn.Module):
    def __init__(self, args: dict, tokenizer: object, data_name: str):
        super(GlobalPretrain, self).__init__()
        self.args = args
        self.tokenizer = tokenizer

        # ==========define model modules===================#
        # define visual encoder
        assert args['visual_encoder'] in ['resnet101']
    ...
    def forward(self, images, radgraph_ids, radgraph_masks):
        ...
        return {
            'itc_region': torch.zeros((1,)),
            'itc_instance': itc_instance_loss,
            'all_loss': itc_instance_loss
        }
