...
class TextEncoderModel(nn.Module):
    ...
    def __init__(self, config, tokenizer):
        super().__init__()
        if config['scibert_path'] is not None:
            enc_config = AutoConfig.from_pretrained(config['scibert_path'])
            ...
            self.encoder = AutoModel.from_pretrained(config['scibert_path'], config=enc_config,
                                                     ignore_mismatched_sizes=True)
        else:
