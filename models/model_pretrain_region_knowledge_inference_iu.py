...
class PretrainInference(nn.Module):
    ...
    def forward(self, images, data_name='mimic_cxr'):
        if data_name == 'mimic_cxr':
            fc_feats, att_feats = self.visual_forward_mimic_cxr(images)
        else:   # iu_xray
            fc_feats, att_feats = self.visual_forward_iu_xray(images)
