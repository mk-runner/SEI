...
class Inference(nn.Module):
    ...
    def text_decoder_forward_r2gen(self, encoder_hidden_states, encoder_attention_mask):
        output, _ = self.text_decoder(encoder_hidden_states, encoder_attention_masks=encoder_attention_mask,
                                      mode='sample')
        gen_texts= []
        for pred in output.cpu().tolist():
            gen_text = self.tokenizer.decode(pred)
            gen_texts.append(gen_text)
        return gen_texts

    def forward(self, images, sk_ids, sk_masks):
        # image embedding
        ...
        ret = self.text_decoder_forward(encoder_hidden_states, encoder_attention_mask)
        return ret
