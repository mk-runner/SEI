...
class FTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        print(f"Epoch {epoch}, training dataset with indications and similar historical cases++++++++++++++++++++++++++++++++++++++")
        self.logger.info(f"Epoch {epoch}, training dataset with indication section and similar historical cases++++++++++++++++++++++++++++++++++++++")
        ...
        with torch.no_grad():
            ...
            if self.val_loader_inc is not None:
                print(f"Epoch {epoch}, validation dataset with indication section and similar historical cases+++++++++++++++++++")
                self.logger.info(f"Epoch {epoch}, validation dataset with indication section and similar historical cases+++++++++++++++++++")
            ...
            val_met = self.metric_ftns(gts=val_gts, gens=val_res, args=self.args)
        ...
        with torch.no_grad():
            if self.test_loader_inc is not None:
                ...
                print(f"Epoch {epoch}, test dataset with indication section and similar historical cases+++++++++++++++++++")
                self.logger.info(f"Epoch {epoch}, test dataset with indication section and similar historical cases+++++++++++++++++++")
                for _, (images_id, images, reports_ids, reports_masks, sk_ids, sk_masks, inc_ids, inc_masks) in enumerate(
            ...
            test_met = self.metric_ftns(gts=test_gts, gens=test_res, args=self.args)
...
class Tester(BaseTrainer):
    ...
    def test(self):
        ...
        with torch.no_grad():
            if self.test_loader_inc is not None:
                test_gts, test_res, test_images_ids = [], [], []
                print(f"test dataset with indication section and similar historical cases+++++++++++++++++++")
                self.logger.info(f"test dataset with indication section and similar historical cases+++++++++++++++++++")
                for _, (images_id, images, reports_ids, reports_masks, sk_ids, sk_masks, inc_ids, inc_masks) in enumerate(
                        self.test_loader_inc):
                    images = images.to(self.device)
                    reports_ids, reports_masks = reports_ids.to(self.device), reports_masks.to(self.device)
