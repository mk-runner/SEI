...
def pack_wrapper(module, att_feats, att_masks):
    if att_masks is not None:
        ...
        return module(att_feats * att_masks.unsqueeze(-1))
    else:
...
class AttModel(CaptionModel):
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)  # begin
    ...
    def _sample(self, encoder_hidden_states, encoder_attention_masks=None):
        fc_feats, att_feats = encoder_hidden_states[:, 0, :], encoder_hidden_states[:, 1:, :]
        fc_masks, att_masks = encoder_attention_masks[:, 0], encoder_attention_masks[:, 1:]
        opt = self.args
