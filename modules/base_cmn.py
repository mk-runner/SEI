...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        embeddings = self.tgt_embed(tgt) # (16, 59, 512)text embedding

        # Memory querying and responding for textual features (16, 2048, 512)
        ...
        # embeddings and tgt_mask for text; memory and src_mask for image
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, past=None):
        if past is not None:  # x is text, memory is image
            present = [[], []]
            x = x[:, -1:]
...
class BaseCMN(AttModel):
    ...
    def _prepare_feature(self, enc_fc_feats, enc_att_feats, enc_att_masks):
        _, enc_att_feats, _, enc_att_masks = self._prepare_feature_forward(enc_att_feats=enc_att_feats,
                                                                           enc_att_masks=enc_att_masks)
        memory = self.model.encode(enc_att_feats, enc_att_masks)
        return enc_fc_feats[..., :1], enc_att_feats[..., :1], memory, enc_att_masks

    def _prepare_feature_forward(self, enc_att_feats, input_ids=None, enc_att_masks=None, attention_mask=None):
        enc_att_feats, enc_att_masks = self.clip_att(enc_att_feats, enc_att_masks)
        enc_att_feats = pack_wrapper(self.att_embed, enc_att_feats, enc_att_masks)

        if enc_att_masks is None:
            enc_att_masks = enc_att_feats.new_ones(enc_att_feats.shape[:2], dtype=torch.long)

        # Memory querying and responding for visual features # expand means each sample has a memory matrix
        dummy_memory_matrix = self.memory_matrix.unsqueeze(0).expand(enc_att_feats.size(0), self.memory_matrix.size(0),
                                                                     self.memory_matrix.size(1))
        responses = self.cmn(enc_att_feats, dummy_memory_matrix, dummy_memory_matrix)
        enc_att_feats = enc_att_feats + responses  # introduce memory network using sum
        enc_att_masks = enc_att_masks.unsqueeze(-2)  # ensure encoder_hidden_states is one token

        # based on the number of words
        if input_ids is not None:
            attention_mask = attention_mask.unsqueeze(-2) & subsequent_mask(input_ids.size(-1)).to(input_ids)

        return input_ids, enc_att_feats, attention_mask, enc_att_masks

    def _forward(self, input_ids, encoder_hidden_states, attention_mask=None, encoder_attention_mask=None):
        enc_fc_feats, enc_att_feats = encoder_hidden_states[:, 0, :], encoder_hidden_states[:, 1:, :]
        enc_fc_masks, enc_att_masks = encoder_attention_mask[:, 0:1], encoder_attention_mask[:, 1:]
        input_ids, enc_att_feats, attention_mask, enc_att_masks = self._prepare_feature_forward(enc_att_feats,
                                                                                                input_ids,
                                                                                                enc_att_masks,
                                                                                                attention_mask)
        out = self.model(enc_att_feats, input_ids, enc_att_masks, attention_mask, memory_matrix=self.memory_matrix)
