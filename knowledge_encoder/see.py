...
class RadGraphNER:
    ...
    def __init__(self, corpus=None, ann_path=None, model_path=None, cuda='1', is_get_output=True, is_mimic=False):
        ...
        self.cuda = cuda
        # user defined
        # self.input_path or self.output_path are the files of inputs and outputs for radgraph
...
def extract_mimic_factual_entity_sequence(mimic_ann_path, radgraph_model_path, new_ann_file_name):
    # radgraph_model_path = '/home/miao/data/dataset/checkpoints/radgraph/model.tar.gz'

    # extract mimic-cxr factual serialization
    radgraph = RadGraphNER(ann_path=mimic_ann_path, is_get_output=True, is_mimic=True, model_path=radgraph_model_path)
    factual_serialization = radgraph.preprocess_mimic_radgraph_output()
    # save the results of factual entity sequence for each report
    get_mimic_cxr_annotations(mimic_ann_path, factual_serialization, new_ann_file_name)
...
if __name__ == '__main__':
    ...
    # and its code is MK13. Note that the "annotation.json" is identical to "mimic_annotation.json"
    ann_path = os.path.join(root, 'annotation.json')
    extract_mimic_factual_entity_sequence(ann_path, radgraph_model_path, new_ann_file_name)

    # extract factual_entity_sequence for a string
